...
/* global Stripe */
  ...
  submitHandler = function (event)  {
    ...
    if(Stripe){
      ...
      show_error("Failed to load credit processing functionality. Please reload the page");
    }
    return false;
  };
  ...
  // function to set up plan change event listener #tenant_plan id in the forms for class cc_form
  $("#tenant_plan").on('change', function(event) {
    handlePlanChange($('#tenant_plan :selected').val(), ".cc_form");
  });
  
  // function to call plan change handler so that the plan is set correctly in the drop down when the page loads
